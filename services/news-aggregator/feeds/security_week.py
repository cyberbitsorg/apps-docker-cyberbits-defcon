...
class SecurityWeekFeed(BaseFeedParser):
    source_id = "security_week"
    ...
    async def fetch(self) -> list[RawArticle]:
        try:
            ...
            logger.error(f"[SW] Failed to fetch feed: {e}")
            return []
        ...
        for entry in feed.entries:
            try:
                ...
                logger.warning(f"[SW] Skipping entry: {e}")

        logger.info(f"[SW] Fetched {len(articles)} articles")
        return articles
