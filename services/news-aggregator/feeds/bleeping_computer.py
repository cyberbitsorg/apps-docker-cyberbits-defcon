...
class BleepingComputerFeed(BaseFeedParser):
    ...
    async def fetch(self) -> list[RawArticle]:
        try:
            ...
            logger.error(f"[BC] Failed to fetch feed: {e}")
            return []
        ...
        for entry in feed.entries:
            try:
                ...
                logger.warning(f"[BC] Skipping entry: {e}")

        logger.info(f"[BC] Fetched {len(articles)} articles")
        return articles
